...
class conv_layer(object):
    def __init__(self, input_x, is_training, in_channel, out_channel, kernel_shape, padding="SAME", binary=False, stochastic=False, index=0):
...
class fc_layer(object):
    def __init__(self, input_x, is_training, in_size, out_size, binary=False, stochastic=False, index=0):
        # binary: whether to implement the Binary Connect
        # stochastic: whether implement stochastic weight if do Binary Connect
...
def Network(input_x, input_y, is_training, is_drop_out, is_binary, is_stochastic, channel_num, output_size,
    ...
    # 64Conv3-BN-64Conv3-MaxPool2-BN-128Conv3-BN-128Conv3-MaxPool2-BN-256Conv3-BN-256Conv3-MaxPool2-BN-1024fc-1024fc-10fc
    ...
    norm_layer_8 = norm_layer(input_x=fc_layer_2.output(),
                              is_training=is_training,
                              is_drop_out=False,
                              activation_function=None,  # no activation for output
                              index=8)
...
def training(X_train, y_train, X_val, y_val, X_test, y_test, is_binary, is_stochastic, conv_featmap, fc_units, conv_kernel_size,
             pooling_size, lr_start, lr_end, epoch, batch_size, is_drop_out, verbose=False, pre_trained_model=None):
    ...
    # define the variables and parameter needed
    ...
    with tf.Session() as sess:
        ...
        if pre_trained_model is None:
            ...
            for i in range(y_test.shape[0] // 100 + 1):
                a = i * 100
                if a >= y_test.shape[0]:
                    continue  # since number of test images is not multiply of 100
